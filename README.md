...
The whole point lives under [`.claude/`](.claude). To prove it works on something real, the repo
...
└── examples/
    └── e-commerce/       Vela Commerce — Go API + Next.js, the example that exercises it all
```
...
Step. The loop *is* the retry. See [`.claude/references/braid-mental-model.md`](.claude/references/braid-mental-model.md).
...
Adapted from public work, with attribution kept in each agent/skill:
