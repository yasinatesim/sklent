...
import { login, register } from "@/features/web/auth/api/auth";
import { useAuthStore } from "@/features/web/auth/store/authStore";

import styles from "@/features/web/auth/styles/auth.module.scss";
...
const AuthPage = () => {
  ...
  const handleAuthSubmit = async (event: FormEvent) => {
    ...
    showToast(t("welcome"));
    router.push(user?.role === "admin" ? localePath(locale, ROUTES.ADMIN) : `/${locale}`);
  };
  ...
  return (
    <main className="container">
      <div className={styles.box}>
        ...
        {!isRegister ? (
          <div className={styles.switch}>
            <a href={localePath(locale, ROUTES.FORGOT_PASSWORD)}>{t("forgotPasswordLink")}</a>
          </div>
        ) : null}
